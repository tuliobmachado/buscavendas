package br.com.informsistemas.buscavendas.controller.fragments;
...
import br.com.informsistemas.buscavendas.controller.adapter.MaterialSearchAdapter;
...
import br.com.informsistemas.buscavendas.model.helper.CalculoClass;
...
public class MaterialSearchFragment extends Fragment implements IOnBackPressed, MaterialSearchAdapter.OnMaterialListener {
    ...
    public void onMaterialClick(int position, BigDecimal quantidade) {
        Material material = null;
        Material materialExclusao = null;
        BigDecimal saldoTotal = listMaterial.get(position).saldodisponivel;
        BigDecimal proxSaldo = listMaterial.get(position).quantidade;
        proxSaldo = proxSaldo.add(quantidade);
        BigDecimal vQuantidade = new BigDecimal(0);
        boolean vQuantidadeFracionada = false;

        if (proxSaldo.floatValue() > saldoTotal.floatValue()){
            Toast.makeText(getActivity(), "Saldo ultrapassado!", Toast.LENGTH_SHORT).show();
            return;
        }
